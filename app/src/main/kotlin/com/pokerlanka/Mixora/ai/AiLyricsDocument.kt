...
object AiLyricsDocumentParser {
    ...
    private fun isTtml(rawLyrics: String): Boolean {
        val trimmed = rawLyrics.trimStart()
        return trimmed.startsWith("<tt", ignoreCase = true) ||
            trimmed.startsWith("<?xml", ignoreCase = true) && rawLyrics.contains("<tt", ignoreCase = true)
    }
            ...
            for (index in 0 until elements.length) {
                ...
                val lineKey =
                    readAttributeBySuffix(element, "key")
                        ?: "mixora-translation-$segmentId".also { key -> element.setAttribute("key", key) }
...
private data class TtmlLyricsDocument(
    ...
    override fun rebuild(translations: Map<Int, String>): String {
        removeMixoraTranslationElements(document.documentElement)
        ...
        if (translatedParagraphs.isNotEmpty()) {
            ...
            val translationElement =
                document.createElement("translation").apply {
                    setAttribute("data-mixora", "translation")
                }
    ...
    private fun removeMixoraTranslationElements(root: Element) {
        ...
        for (index in 0 until elements.length) {
            ...
            if (element.tagName.endsWith("translation", ignoreCase = true) &&
                element.getAttribute("data-mixora") == "translation"
            ) {
