...
class AiIntegrationSettingsViewModel
    ...
    constructor(
        ...
        fun validateAndLoadModels(
            provider: AiProvider,
            apiKey: String,
            customEndpoint: String,
        ) {
            if (provider == AiProvider.NONE || apiKey.isBlank()) return
            viewModelScope.launch(Dispatchers.IO) {
                _actionState.value =
                    _actionState.value.copy(
                        isTesting = true,
        ...
        fun testApi() {
            ...
            viewModelScope.launch(Dispatchers.IO) {
                ...
                try {
                    ...
                    _actionState.value = _actionState.value.copy(errorMessage = null)
                    _events.emit(context.getString(R.string.ai_api_connected))
                    if (config.provider != AiProvider.CUSTOM && _availableModels.value.isEmpty()) {
                        fetchModels(config.provider, config.apiKey, config.customEndpoint)
                    }
                } catch (e: Exception) {
                    context.dataStore.edit { prefs ->
                        prefs[AiApiValidationStatusKey] = AiApiValidationStatus.FAILED.name
