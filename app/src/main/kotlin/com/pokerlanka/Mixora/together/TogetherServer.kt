...
class TogetherServer(
    ...
    suspend fun approveParticipant(
        ...
        if (!approved) {
            ...
            runCatching {
                client.session.send(
                    TogetherJson.encodeToString(
                        TogetherMessage.serializer(),
                        JoinDecision(sessionId = sessionId, participantId = participantId, approved = false),
                    ),
                )
                client.session.close(CloseReason(CloseReason.Codes.CANNOT_ACCEPT, "Host rejected join request"))
            }
            onEvent?.invoke(TogetherServerEvent.ParticipantLeft(participantId, "Rejected"))
            return
        }
    ...
    suspend fun kickParticipant(participantId: String) {
        val client = clients[participantId] ?: return
        clients.remove(participantId)
        runCatching {
            client.session.send(
                TogetherJson.encodeToString(
                    TogetherMessage.serializer(),
                    KickParticipant(sessionId = sessionId, participantId = participantId, reason = "Kicked by host"),
                ),
            )
            client.session.close(CloseReason(CloseReason.Codes.NORMAL, "Kicked"))
        }
        onEvent?.invoke(TogetherServerEvent.ParticipantLeft(participantId, "Kicked"))
    }
    ...
    suspend fun banParticipant(participantId: String) {
        ...
        runCatching {
            client.session.send(
                TogetherJson.encodeToString(
                    TogetherMessage.serializer(),
                    BanParticipant(sessionId = sessionId, participantId = participantId, reason = "Banned by host"),
                ),
            )
            client.session.close(CloseReason(CloseReason.Codes.NORMAL, "Banned"))
        }
        onEvent?.invoke(TogetherServerEvent.ParticipantLeft(participantId, "Banned"))
    }

    suspend fun broadcastRoomState(state: TogetherRoomState) {
        val snapshotSettings = mutex.withLock { settings }
        val activeHostId = mutex.withLock { authorityParticipantId } ?: hostParticipantId
        rebuildParticipantsSnapshot(activeHostId)
    ...
    private fun rebuildParticipantsSnapshot(activeHostId: String) {
        val host =
            TogetherParticipant(
    ...
    private suspend fun broadcastParticipantJoined(participant: TogetherParticipant) {
        val message =
            TogetherJson.encodeToString(
                TogetherMessage.serializer(),
                ParticipantJoined(sessionId = sessionId, participant = participant),
            )
        clients.values.forEach { client ->
            if (!client.pending && client.participantId != participant.id) {
    ...
    private suspend fun WebSocketSession.handleClient() {
        ...
        val hello =
            runCatching {
                ...
                TogetherJson.decodeFromString<TogetherMessage>(txt) as? ClientHello
            }.getOrNull()
        ...
        send(TogetherJson.encodeToString(TogetherMessage.serializer(), welcome))
        ...
        try {
            for (frame in incoming) {
                ...
                val message =
                    runCatching { TogetherJson.decodeFromString<TogetherMessage>(text) }
                        .getOrElse {
